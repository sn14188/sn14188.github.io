...
### Overview
Express로 서버를 만들고, 사용자가 로그인하면 정해진 쿠키가 발급되도록 설정했습니다.<br>
...
2. 디폴트는 `SameSite=Lax`이며 이 경우 브라우저는 레거시 동작을 적용할 수 있다는 것
   - `SameSite`를 명시적으로 지정하지 않은 쿠키는 `SameSite=None`인 것처럼 처리됩니다
   - 이는 same-site와 cross-site 요청 모두에서 전송이 된다는 뜻입니다

## Removing the SameSite Option
이 사실들을 알고 난 뒤, 아예 `sameSite` 옵션을 제거한 뒤 다시 테스트했습니다.<br>
이번에는 `attacker` 페이지에서 쿠키가 정상적으로 전송되는 것을 확인할 수 있었습니다!<br>
<br>(이미지)<br>

## Testing Secure Cookie on Localhost
...
잠재적으로 신뢰할 수 있는 출처란, 엄밀히 보안 기준을 충족하지 못하지만 브라우저가 일반적으로 데이터 보안을 제공할 수 있다고 믿을 수 있는 오리진이라 합니다. 로컬에서 전송되는 리소스 및 `URL`은 `HTTPS`를 통해 전송되지 않지만, 브라우저와 동일한 기기에 있기 때문에 안전하게 전송되는 걸로 간주할 수 있다고 합니다.

## Takeaways
1. `Samesite=None`과 `Secure=false` 조합은 브라우저가 거부하는 동작일 수 있다
2. `SameSite`를 지정하지 않으면 브라우저가 레거시 모드로 처리한다
3. `Secure` 쿠키는 원칙적으로 `HTTPS`에서만 전송되지만, 브라우저는 `localhost`를 잠재적으로 신뢰할 수 있는 출처로 판단해 쿠키가 전송될 수 있다
<br><br>
...
[4] MDN Docs (["Secure contexts"](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts))<br>*
