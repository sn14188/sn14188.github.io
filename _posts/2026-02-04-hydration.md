...
이번 포스트에서는 hydration의 개념과, 정적 렌더링이나 SSR 환경에서 hydration의 역할, hydration 이슈와 대응 방안에 대해 살펴봤습니다.
<br><br>
...
6. CSS-in-JS 라이브러리 설정이 잘못된 경우
   - CSS-in-JS는 서버와 클라이언트에서 `className`을 생성하는 방식이 어긋나면 HTML이나 속성이 달라져 mismatch를 유발할 수 있습니다
   - 대응 방안: 프레임워크 공식 문서에서 제공하는 SSR 설정을 잘 따름으로써 방지할 수 있습니다

7. Edge/CDN가 HTML 응답을 변경하는 경우
   - Edge나 CDN은 중간 인프라로 사용자와 서버 사이에 위치하면서, 요청과 응답을 전달하는 과정에서 HTML 응답을 가공할 수 있습니다
